...
#POST param
effectValue=$1
updateDocFrDt=${2:-"年月日 [YYYYMMDD]"} #指定時は8桁数字19801028
updateDocToDt=${3:-"年月日 [YYYYMMDD]"} #指定時は8桁数字19801028

#output param
d=${2:-"00000000"}_${3:-""}
filename=${1:-all}

mkdir -p searchresult/$d

http \
--ignore-stdin \
--form \
--output searchresult/$d/${filename}.html \
POST \
...
dispColumnsList[15]= \
changeColumnsList= \
updateDocFrDt="${updateDocFrDt}" \
updateDocToDt="${updateDocToDt}" \
tglOpFlg= \
compNameWord= \
koumoku1Value= \
koumoku2Value= \
